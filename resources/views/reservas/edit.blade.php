<x-app-layout>
    <div class="p-8 max-w-lg mx-auto">
        <h1 class="text-2xl font-bold mb-4 text-purple-800">Editar Reserva</h1>

        <form action="{{ route('reservas.update', $reserva) }}" method="POST" class="space-y-3">
            @csrf
            @method('PUT')
            ...
            <select name="usuario_id" id="usuario_id" class="w-full border p-2 rounded" required>
                @foreach($usuarios as $usuario)
                    <option value="{{ $usuario->id }}" {{ $reserva->usuario_id == $usuario->id ? 'selected' : '' }}>{{ $usuario->name }} ({{ $usuario->email }})</option>
                @endforeach
            </select>
            ...
            <select name="funcion_id" id="funcion_id" class="w-full border p-2 rounded" required>
                @foreach($funciones as $funcion)
                    <option value="{{ $funcion->id }}" 
                            ...
                            {{ $reserva->funcion_id == $funcion->id ? 'selected' : '' }}>
            ...
            <input type="number" name="cantidad_boletos" id="cantidad_boletos" value="{{ old('cantidad_boletos', $reserva->cantidad_boletos) }}" min="1" class="w-full border p-2 rounded" required>

            <label for="asientos">Asientos (ej: A1,B2,C3)</label>
            <input type="text" name="asientos" value="{{ old('asientos', $reserva->asientos) }}" class="w-full border p-2 rounded" required 
                   placeholder="A1,B2,C3" pattern="^[A-Z][0-9](,[A-Z][0-9])*$"
                   title="Ingresa los asientos separados por coma (ej: A1,B2,C3)">

            <label for="total">Total</label>
            <input type="number" name="total" id="total" step="0.01" value="{{ old('total', $reserva->total) }}" class="w-full border p-2 rounded bg-gray-100" required readonly>

            <label for="pago">Método de pago</label>
            <select name="pago" class="w-full border p-2 rounded" required>
                <option value="efectivo" {{ $reserva->pago == 'efectivo' ? 'selected' : '' }}>Efectivo</option>
                <option value="tarjeta" {{ $reserva->pago == 'tarjeta' ? 'selected' : '' }}>Tarjeta</option>
            </select>

            <button type="submit" class="bg-purple-700 text-white px-4 py-2 rounded">Actualizar Reserva</button>
        </form>
    </div>
